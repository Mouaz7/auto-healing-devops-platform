...
PR body rendering is delegated to:
  - pr_body_builder.py  — full markdown assembly
  - pr_bug_table.py     — bug → fix table builders
"""
...
from src.gerrit_mcp.pr_body_builder import build_pr_body
...
class PatchSubmitter:
    ...
    async def _open_pr(
        ...
        """Build the PR body and open the pull request on GitHub."""
        import base64 as _b64

        rd            = report_data or {}
        original_code = rd.get("original_code", "")

        # Fetch original code from the base branch if not already in report_data
        ...
        body = build_pr_body(rd, patch, build_id, affected_files, original_code)
