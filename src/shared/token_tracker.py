...
class TokenTracker:
    ...
    def check_budget(self, agent_name: str, requested_tokens: int) -> bool:
        """Check if the agent has budget for requested_tokens.

        Args:
            agent_name: Name of the agent (must be in AGENT_CONFIGS).
            requested_tokens: How many tokens the agent wants to use.
    ...
    def record_usage(self, agent_name: str, tokens_used: int) -> None:
        ...
        Args:
            ...
            tokens_used: How many tokens were actually consumed.
        """
        self._reset_if_new_hour()
    ...
    def get_remaining(self, agent_name: str) -> int:
        """Return remaining tokens for agent this hour.

        Args:
            agent_name: Name of the agent.

        Returns:
            Number of tokens remaining in the current hour window.
        """
        self._reset_if_new_hour()
        config = AGENT_CONFIGS[agent_name]
    ...
    def get_used(self, agent_name: str) -> int:
        """Return tokens used by agent this hour."""
        self._reset_if_new_hour()
